...
def listener_configurer_belt():
    # logging.getLogger("pika").propagate = False

    root = logging.getLogger()
    h = logging.handlers.RotatingFileHandler('belt_temp.log', 'a', 3000000, 10)
    f = logging.Formatter('%(asctime)s %(processName)-8s %(name)s %(levelname)-8s %(message)s')
    h.setFormatter(f)
    root.addHandler(h)
...
def listener_process(queue, configurer):
    configurer()
    warning_type = '安全帽'
    if configurer.__name__ == "listener_configurer_jacket":
        warning_type = '救生衣'
    elif  configurer.__name__ == "listener_configurer_belt":
        warning_type = '安全带'
    while True:
        try:
            record = queue.get()
...
def warning_processor(logger, record, warning_type):
    ...
    msg = area + ' 发生未佩戴' + warning_type +  '进入区域'
