...
def image_get(q, ip, channel,rect, log_queue):
    ...
    logger = logging.getLogger(str(ip)+','+str(channel))
    # level = choice(LEVELS)
...
def run_multi_camera():
    ...
    for queue, rtsp in zip(queues, rtsps):
        ...
        processes.append(mp.Process(target=image_get, args=(queue, camera_ip, ch, url_rect_dict[full_vedio_url], log_queue)))
